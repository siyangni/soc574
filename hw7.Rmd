...
The sample correlation between the two is about 0.12 (rounded to the nearest hundredth). We should expect to see this result, because we just saw a very weak relationship between the two from the scatter plot above.
...
The OLS estimate of $\beta_{1}$ is 0.21. It is significant at the $\alpha$ = .05 level. 
...
```
Since our F-statistics is 43, which is larger than 3.85, the critical value at $\alpha$ = .05. We should the reject the mull hypothesis. 
...
From the regression output below, we can see that the OLS estimate of $\beta_{1}$ is about 0.27. It's significant different from zero at the $\alpha$ = .05 level. 

The $\beta_{1}$ is different because here in our model, $\beta_{1}$ represents the linearly estimated relationship between experience and wages, controlling education. 

The results we have here should "better" represents the relationship in the real world, because education affects wages greatly according to theory and our intuition. After controlling education, we did see a stronger relationship between wage and experience, to which we're also expecting.    
...
From the regression table presented above for question b), we can see that $\beta_{2}$ is significant at the $\alpha$ = 0.05 level. The t-statistics is 15.090, much larger than the cut-off t-statistics for $\alpha$ = 0.05, which we calculated below:
...
```
From the table above, we can see that F is also 227.71, and it is significant  at the .05 level. We reject the null hypothesis ($\beta_{2}=0$) and conclude that the column `education` contributes significant information to wages.  

The t-test and partial F-test here render the same conclusion. Actually, we can see that the square of the t-statistics for education equals to the F-statistics we have our partial F-test. 
...
To test this hypothesis, we simply refer to the F-statistics for our full model:

```{r}
summary(w_e_edu_lm)
```

The F-statistics is 137, significant at the .05 level, indicating that we should clearly reject the hull hypothesis that the variables `experience` and `educ` collectively have no effect on `wages`. 

The result also shows that the variable `experience` is significant (p < 2e-16) controlling for the variable `educ`, as is `educ` (p < 2e-16) controlling for the variable `experience`

### e)	
Test the null hypothesis H0: β1 + β2 = 2 at the α = 0.05 level.

To test this, we simply pull out the 95% confidence interval for both $\beta_{1}$ and $\beta_{2}$, and them up to construct the 95% confidence interval for $\beta_{1} + \beta_{2}$:
